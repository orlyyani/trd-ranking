-- ============================================================
-- Migration 016: Doubles match support
-- ============================================================
--
-- Adds match_type ('singles' | 'doubles') to matches.
-- For doubles: player1+player3 = Team A, player2+player4 = Team B.
--              winner_id = captain of winning team (player1 or player2).
--
-- Doubles MMR: team average MMR used for expected score.
--              K-factor = half of singles (K=20 first 10 matches, K=10 after).
-- Separate doubles_wins / doubles_losses on players — singles wins/losses untouched.
-- ============================================================
...
ALTER TABLE public.matches
  ...
  ADD COLUMN player3_id uuid REFERENCES public.players(id),  -- partner of player1 (doubles only)
  ADD COLUMN player4_id uuid REFERENCES public.players(id);  -- partner of player2 (doubles only)
...
-- ── 3. Updated recalculate_all_mmr() ─────────────────────────────────────────
--
-- Singles:  unchanged logic (K=40/20, updates wins/losses)
-- Doubles:  team avg MMR, K=20/10 per player, updates doubles_wins/doubles_losses
--           Total match count (singles+doubles) determines which K bracket a player is in.
...
BEGIN
  ...
  -- 2. Replay every completed match in strict chronological order.
  FOR rec IN
  ...
  LOOP
    ...
    THEN
      ...
      -- Total match count drives K selection (all types count toward experience).
      ...
      -- K=20 first 10 matches, K=10 after (half of singles).
      new_wt1_mmr := wt1_mmr + ROUND((CASE WHEN wt1_mc < 10 THEN 20 ELSE 10 END)::float * (1.0 - exp_win));
      new_wt2_mmr := wt2_mmr + ROUND((CASE WHEN wt2_mc < 10 THEN 20 ELSE 10 END)::float * (1.0 - exp_win));
      new_lt1_mmr := lt1_mmr + ROUND((CASE WHEN lt1_mc < 10 THEN 20 ELSE 10 END)::float * (0.0 - exp_los));
      new_lt2_mmr := lt2_mmr + ROUND((CASE WHEN lt2_mc < 10 THEN 20 ELSE 10 END)::float * (0.0 - exp_los));
    ...
    ELSE
      ...
      k_winner := CASE WHEN winner_mc < 10 THEN 40 ELSE 20 END;
      k_loser  := CASE WHEN loser_mc  < 10 THEN 40 ELSE 20 END;
